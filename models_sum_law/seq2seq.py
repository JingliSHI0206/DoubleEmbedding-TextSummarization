...
import utils
from models_sum_med.lstm import *
from models_sum_med.beam import *
import random


class seq2seq(nn.Module):

    def __init__(self, args, emb_word=None, emb_law=None,  use_attention=True, encoder=None, decoder=None):
        super(seq2seq, self).__init__()

        if encoder is not None:
            self.encoder = encoder
        else:
            self.encoder = rnn_encoder(args, emb_word=emb_word, emb_law=emb_law)
        tgt_embedding = self.encoder.embedding
        if decoder is not None:
            self.decoder = decoder
        else:
            self.decoder = rnn_decoder(args, embedding=tgt_embedding, emb_word=emb_word, emb_law=emb_law,  use_attention=use_attention)
    ...
    def beam_sample(self, src, src_len, beam_size=1, eval_=False):
        ...
        beam = [Beam(beam_size, n_best=1, cuda=self.use_cuda, length_norm=True)
